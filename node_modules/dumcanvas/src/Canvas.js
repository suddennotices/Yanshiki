...
class Canvacord {
     ...
     * @param {String} color Hex or HTML5 color name or rgb
     * @param {String} backgroundd Background URL
     * @returns {Promise<Buffer>}
     * @example let img = await canva.welcome({ username: "Snowflake", discrim: "0007", avatarURL: "..." });
     * canva.write(img, "img.png");
     */
    async welcome({ username, discrim, avatarURL, backgroundd }) {
        ...
        if (!backgroundd) {
            backgroundd = (__dirname + '/assets/images/welcomebg.png')
        }
        ...
        const background = await Canvas.loadImage(backgroundd);
        ctx.drawImage(background, 0, 0, canvas.width, canvas.height);
     ...
     * canva.write(img, "img.png");
     */
    async leaver({ username, discrim, avatarURL, color, backgroundd }) {
        if (!username) throw new Error('No username was provided!');
        if (!discrim) throw new Error('No discrim was provided!');
        if (!avatarURL) throw new Error('No avatarURL was provided!');
if (!backgroundd) {
    backgroundd = (__dirname + '/assets/images/leavebg.png')
}
        Canvas.registerFont(__dirname + '/assets/fonts/regular-font.ttf', {
            family: 'Manrope',
            weight: 'regular',
        ...
        const background = await Canvas.loadImage(__dirname + '/assets/images/leavebg.png');
        ctx.drawImage(background, 0, 0, canvas.width, canvas.height);
